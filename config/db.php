...
function base_seed(PDO $pdo): void {
    if ((int)$pdo->query('SELECT COUNT(*) FROM admin')->fetchColumn() === 0) {
        $pdo->prepare('INSERT INTO admin(name,email,password_hash,role) VALUES(?,?,?,?)')
            ->execute(['Cyber Sathi Admin', 'admin@cybersathi.org', password_hash('admin123', PASSWORD_BCRYPT), ROLE_ADMIN]);
    }

    $s = $pdo->prepare('SELECT COUNT(*) FROM admin WHERE email=?');
    $s->execute([SUPER_ADMIN_EMAIL]);
    if ((int)$s->fetchColumn() === 0) {
        $pdo->prepare('INSERT INTO admin(name,email,password_hash,role) VALUES(?,?,?,?)')
            ->execute(['Super Admin', SUPER_ADMIN_EMAIL, password_hash(SUPER_ADMIN_DEFAULT_PASSWORD, PASSWORD_BCRYPT), ROLE_SUPER_ADMIN]);
    }
...
function bootstrap_mysql(PDO $pdo): void {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS admin(
        ...
        role ENUM('super_admin','state_cyber_cell','district_cyber_cell','admin') DEFAULT 'admin',
        state_id INT NULL,
        district_id INT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT fk_admin_state FOREIGN KEY (state_id) REFERENCES states(id) ON DELETE SET NULL,
        CONSTRAINT fk_admin_district FOREIGN KEY (district_id) REFERENCES districts(id) ON DELETE SET NULL
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS volunteers(
        ...
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT fk_vol_state FOREIGN KEY (state_id) REFERENCES states(id) ON DELETE SET NULL,
        CONSTRAINT fk_vol_district FOREIGN KEY (district_id) REFERENCES districts(id) ON DELETE SET NULL
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS complaints(
        ...
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT fk_complaint_volunteer FOREIGN KEY (assigned_to) REFERENCES volunteers(id) ON DELETE SET NULL,
        CONSTRAINT fk_complaint_state FOREIGN KEY (state_id) REFERENCES states(id) ON DELETE SET NULL,
        CONSTRAINT fk_complaint_district FOREIGN KEY (district_id) REFERENCES districts(id) ON DELETE SET NULL
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
    ...
    try {$pdo->exec("ALTER TABLE admin MODIFY role ENUM('super_admin','state_cyber_cell','district_cyber_cell','admin') DEFAULT 'admin'");} catch (Throwable $e) {}
    try {$pdo->exec('ALTER TABLE admin ADD COLUMN state_id INT NULL');} catch (Throwable $e) {}
    try {$pdo->exec('ALTER TABLE admin ADD COLUMN district_id INT NULL');} catch (Throwable $e) {}
    ...
    try {$pdo->exec('ALTER TABLE users ADD COLUMN state_id INT NULL');} catch (Throwable $e) {}
    try {$pdo->exec('ALTER TABLE users ADD COLUMN district_id INT NULL');} catch (Throwable $e) {}
    try {$pdo->exec('ALTER TABLE volunteers ADD COLUMN state_id INT NULL');} catch (Throwable $e) {}
    try {$pdo->exec('ALTER TABLE volunteers ADD COLUMN district_id INT NULL');} catch (Throwable $e) {}
    try {$pdo->exec('ALTER TABLE complaints ADD COLUMN state_id INT NULL');} catch (Throwable $e) {}
    try {$pdo->exec('ALTER TABLE complaints ADD COLUMN district_id INT NULL');} catch (Throwable $e) {}
...
function bootstrap_sqlite(PDO $pdo): void {
    $pdo->exec("CREATE TABLE IF NOT EXISTS states(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT UNIQUE,created_at TEXT DEFAULT CURRENT_TIMESTAMP)");
    $pdo->exec("CREATE TABLE IF NOT EXISTS districts(id INTEGER PRIMARY KEY AUTOINCREMENT,state_id INTEGER,name TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP)");
    $pdo->exec("CREATE TABLE IF NOT EXISTS admin(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT,email TEXT UNIQUE,password_hash TEXT,role TEXT DEFAULT 'admin',state_id INTEGER,district_id INTEGER,created_at TEXT DEFAULT CURRENT_TIMESTAMP)");
    $pdo->exec("CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT,email TEXT UNIQUE,mobile TEXT,city TEXT,state_id INTEGER,district_id INTEGER,password_hash TEXT,login_type TEXT DEFAULT 'normal',is_verified INTEGER DEFAULT 0,created_at TEXT DEFAULT CURRENT_TIMESTAMP)");
    $pdo->exec("CREATE TABLE IF NOT EXISTS volunteers(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT,email TEXT UNIQUE,mobile TEXT,city TEXT,state_id INTEGER,district_id INTEGER,skills TEXT,occupation TEXT,password_hash TEXT,approval_status TEXT DEFAULT 'pending',volunteer_role TEXT DEFAULT 'Cyber Volunteer',created_at TEXT DEFAULT CURRENT_TIMESTAMP)");
    $pdo->exec("CREATE TABLE IF NOT EXISTS complaints(id INTEGER PRIMARY KEY AUTOINCREMENT,user_name TEXT,mobile TEXT,email TEXT,city TEXT,state_id INTEGER,district_id INTEGER,fraud_type TEXT,description TEXT,suspected_source TEXT,evidence_path TEXT,status TEXT DEFAULT 'open',assigned_to INTEGER,created_at TEXT DEFAULT CURRENT_TIMESTAMP)");
...
function get_email_settings(PDO $pdo): array {
    try {
        $r = $pdo->query('SELECT * FROM email_settings ORDER BY id DESC LIMIT 1')->fetch();
        return $r ?: [];
    } catch (Throwable $e) {
        return [];
    }
...
try {
    ...
    $sqlitePath = __DIR__ . '/../database/cybersathi.sqlite';
